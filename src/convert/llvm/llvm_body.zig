...
fn processFunctionBodyNode(
    ...
    switch (node.node_type) {
        ...
        .VarDeclaration,
        .ConstDeclaration => try processLocalDecl(allocator, convert_data, node),
        ...
        .FunctionCall,
        .MemberAccess     => try processCallStmt(allocator, convert_data, node),
        .Block,
        .IfBody,
        .ElseBody,
        .LoopBody         => try processBody(allocator, convert_data, node),
        .Comment          => {},
...
fn processLocalDecl(
    ...
    // alloca
    const addr_reg = std.fmt.allocPrint(allocator.*, "%{s}_addr", .{var_name})
    ...
    if (node.right) |val_node| {
        var stmts = std.ArrayList([]const u8).initCapacity(allocator.*, 0)
            catch return ConvertError.Out_Of_Memory;
    ...
    const meta = std.fmt.allocPrint(allocator.*, "{s}|{s}", .{ llvm_type, addr_reg })
        catch return ConvertError.Out_Of_Memory;
...
fn freshLabel(allocator: *Allocator, convert_data: *ConvertData) ConvertError![]u8 {
    const name = std.fmt.allocPrint(allocator.*, "lbl{d}", .{convert_data.tmp_counter})
        catch return ConvertError.Out_Of_Memory;
